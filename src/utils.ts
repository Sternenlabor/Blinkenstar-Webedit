...
type FrameColumns = List<List<boolean>>

const EMPTY_COLUMN: List<boolean> = List(range(8).map(() => false))

const _cache_size = 100
const textColumnsCache: Record<string, FrameColumns> = new Proxy(
    {} as Record<string, FrameColumns>,
    {
        set: (obj, prop, value: FrameColumns) => {
            const key = String(prop)
            // evict prefixes
            Object.keys(obj)
                .reduce<string[]>((acc, cur) => (key.startsWith(cur) ? acc.concat(cur) : acc), [])
                .map((str) => {
                    delete obj[str]
                    console.log('cache: evicted substr', str)
                })
...
export function textColumns(txt: string): FrameColumns {
...
export function getFrameColumns(animation: Animation, frame: number): FrameColumns {
